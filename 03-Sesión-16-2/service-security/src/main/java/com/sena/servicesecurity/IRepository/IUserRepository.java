...
public interface IUserRepository extends IBaseRepository<User, Long> {
	@Query(value = " SELECT " + " u.id,	" + " u.username user, " + " CONCAT(p.first_name,' ',p.last_name) personName, "
			+ " p.email personEmail, " + " u.state " + "FROM " + "	user u "
			+ "    INNER JOIN person p ON p.id = u.person_id " + "WHERE " + "	u.username = :username AND "
